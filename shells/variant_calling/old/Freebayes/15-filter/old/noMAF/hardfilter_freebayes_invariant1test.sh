...
zcat freebayes_invariant_chr01_all.vcf.gz | bcftools filter -g 5 -i 'QUAL/DP > 2.0 || QUAL > 50 || TYPE = "indel" || TYPE = "snp" || TYPE = "mnp" || TYPE = "ref" || SAF > 1 || SAR > 1 || RPR > 1 || RPL > 1' | vcftools --vcf - --maf 0.8 --minDP 3 --max-missing 0.8 --maxDP 25 --recode --stdout | gzip -c > freebayes_invariant_chr01_allFilteredtestMAF.vcf.gz
...
# 
zcat freebayes_invariant_chr01_all.vcf.gz | bcftools filter -g 5 -i 'TYPE = "indel" || TYPE = "snp" || TYPE = "mnp" || TYPE = "ref" ' | bcftools filter -g 5 -i 'QUAL/DP > 2.0 && QUAL > 50 && SAF > 1 && SAR > 1 && RPR > 1 && RPL > 1' | vcftools --vcf - --maf 0.8 --minDP 3 --max-missing 0.8 --maxDP 25 --recode --stdout | /clusterfs/rosalind/users/makman/tabix-0.2.6/bgzip -c > freebayes_invariant_chr01_allFilteredtest.vcf.gz
