## This script uses a sites.txt file from bcftools isec to select variants from one 
## variant calling vcf files. I use this to extract ones that are present only in FB and
## using vcfconcat/vcf-sort I combine it with 0000.vcf.gz which is GATK
import sys
...
sites_count = 0
vcf_count = 0
for line in file2: # reading sites file and creating a dict
	split = line.split("\t")
	chr = split[0][-2:]
	pos = split[1]
	type = split[4][:-1]
...
# 		if type == "110" or type == "101" or type == "011" or type == "111":
	if type == "011": # this is true for FB and ST intersects
