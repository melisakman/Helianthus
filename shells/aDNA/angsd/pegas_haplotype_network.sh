...
Total_lines=`grep -c "^" regions_bed.txt`
...
do
  ...
  sed "${Line_to_read}q;d" regions_bed.txt > Current_gene_tested.bed;
  Line_to_read=`expr $Line_to_read + 1`
	...
	srun vcftools --vcf /global/scratch/makman/Sunflower_angsd/all_angsd.vcf.gz --chr $var_current_gene_chr --from-bp $var_current_gene_start --to-bp $var_current_gene_end --recode --out ${var_current_gene_name}
 	mv ${var_current_gene_name}.recode.vcf /clusterfs/vector/scratch/makman/haplotype_networks/Haplo_by_gene/VCF_per_gene/${var_current_gene_name}.vcf
